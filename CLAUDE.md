...
5. **URL fact-check + OSS enrichment**（見下「URL 抓取路由」） — 按 source 分流（reddit → pullmd / Cloudflare → mcp__fetch / 其他 → WebFetch）+ GitHub `gh repo view`
6. **寫 digest HTML** — 依 v3 範本（header / stat cards / charts / 主軸 sections / 系統當天動態 / footer）
...
4. **`Triggered by: stage-2-digest` / `both` 的 entries — agent 在 digest 啟動時主動即時 fetch**（不依賴 06:45 daily wrapper、避免低頻訊號 spam）：
   ...
   - 範例：`anthropics/claude-code release watcher` entry — 每次 stage-2 digest 都 fetch 最新 release tag、比對 baseline、有新 release 開「🆕 CC CLI 動態」section
