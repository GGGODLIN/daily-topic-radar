...
PROMPT=$(cat <<'EOF'
你是流程固化偵察員。從 session 指紋 ledger 做兩件事：(A) 找「重複出現但尚未固化」的工作流 (B) 對已固化的高頻流程做執行漂移抽查。產出繁中報告到 stdout。只偵察、只建議——**絕不自建或自改任何 skill / command / workflow**，固化與修改由使用者拍板。
...
4. 漂移發現 = 修改提案（改哪支、哪段、為什麼），跟 A 線候選一起進報告。**絕不直接改 skill**——自動改寫固化資產是使用者明確否決的軸
5. 無漂移 → 一行「抽查 X/Y/Z 無顯著漂移」帶過

## Step 7 — 輸出報告

A 線每條候選：工作流一句話描述 + 證據（哪幾天、幾個 session、intent 樣本）+ 建議形態（skill / command / workflow / 「extend 既有 X」）+ 一句為什麼值得固化。
B 線每條漂移：哪支資產 + 漂移類型（跳步/加步/糾正）+ 計數證據 + 修改提案。
**A 線零候選且 B 線無漂移 → 明寫一行帶過，不要硬湊。**
EOF
)
